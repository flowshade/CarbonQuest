...
<main class="main">
    {#if $displayName}
        {#if $displayName.length == 0}
            <div class="p-3" id="editName">
                <input type="text" name="" class="input rounded-xl shadow-xl" bind:value={name} placeholder="What's your name?">
                <button on:click={addProfile} class="button bg-green rounded-xl">Save</button>
            </div>
        {:else}
            ...
            <div class="text-center">
                {#each $displayName as profile (profile.id)}
                    ...
                    <button on:click={shareProfile} class="inline button rounded-xl bg-green">
                        Share 
