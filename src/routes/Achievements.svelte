<script>
    ...
    let getGoals = async () => {
        return await db.goals.toArray().then((data) => {
            return data;
        })
    }

    let streak = getStreak().then(data => data);
    let goals = getGoals().then(data => data)
</script>

<main class="main pb-20">
    <h1 class="m-4 p-4 rounded-lg">Achievements 🏆</h1>
    <div>
        {#each achievements as achievement}
            <section class="section">
                ...
                {#if (achievement.type == "streak")}
                    ...
                    {:then streakData}
                        {#if (streakData / achievement.streakGoal) < 1}
                            <div class="w-full bg-neutral-300 rounded-full h-3 dark:bg-gray-700 mt-3">
                                <div class="bg-green h-3 rounded-full" style="width: {(streakData / achievement.streakGoal) * 100}%"></div>
                            </div>
                        {:else}
